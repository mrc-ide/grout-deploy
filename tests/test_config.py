...
def test_packit(cfg):
    ...
    assert cfg.packit_servers["reside"]["url"] == "https://packit.dide.ic.ac.uk/reside/"
...
def test_get_dataset_tile_levels(cfg):
    assert cfg.datasets.get_dataset_tile_levels("gadm41") == ["admin0", "admin1", "admin2"]


def test_get_tile_level_details(cfg):
    server, packit_id, download = cfg.datasets.get_tile_level_details("gadm41", "admin0")
