...
class GroutConfig:
    def __init__(self, path: str, config_name: str):
        ...
        self.docker_container_name = config.config_string(
        ...
        for server, server_config in packit_config.items():
            self.packit_servers[server] = {
                "url": config.config_string(server_config, ["url"])
            }

        self.datasets = GroutDatasetsConfig(
            config.config_dict(dat, ["datasets"])
        )
